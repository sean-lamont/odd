...
* **`generator.py`**: Contains `DPPGenerator`, a highly efficient text generator that manages the iterative diffusion loop and applies the selected strategy at each timestep. Optimized for fast benchmarking.
* **`app_generator.py`**: Contains `AppDPPGenerator`, a specialized generator used exclusively by the Streamlit app to track counterfactuals and intensive logging metrics.
