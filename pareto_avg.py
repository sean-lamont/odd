...
def plot_pass1_vs_pass16_avg_alpha(json_path):
    # 1. Load Data
    try:
    ...
    # 5. STAGE 3 AGGREGATION: Average over Temperatures (One row per Alpha)
    final_df = alpha_temp_level.groupby('alpha')[['p1_run', 'p16_run']].mean().reset_index()

    # Sort by alpha to trace the trajectory properly
    ...
    # Use a strong, single color for the global trajectory
    trajectory_color = sns.color_palette("mako")[1]

    # Plot the line connecting the alphas
    plt.plot(
        final_df['p1_run'],
        final_df['p16_run'],
        marker='o',
    ...
    # 7. Annotate each point with its Alpha value
    ...
    plt.tight_layout()
    plt.savefig('pareto_batched_gsm_avg_alpha.pdf', dpi=300, bbox_inches='tight')
    plt.show()


# --- Usage ---
if __name__ == "__main__":
    plot_pass1_vs_pass16_avg_alpha('gsm_table_batched.json')