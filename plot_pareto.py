...
def plot_pass1_vs_pass16_corrected(json_path):
    ...
    # 5. Plotting Aesthetics Setup
    plt.figure(figsize=(9, 6.5))

    # Use a clean, minimal theme
    sns.set_theme(style="whitegrid", font_scale=1.1)

    # Create a premium color palette (mako is excellent for scientific plots)
    unique_alphas = sorted(final_df['alpha'].unique())
    ...
    # 7. Zoomed-in bounds with epsilon padding
    ...
    # Optional: Add a subtle text/arrow indicating temperature direction
    # (Placed generally in the bottom right corner so it doesn't overlap data)
    plt.annotate(
    ...
    plt.tight_layout()
    plt.savefig('pareto_batched_he.pdf', dpi=300, bbox_inches='tight')
    plt.show()
