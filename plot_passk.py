...
def plot_pass_at_k_grid_bespoke(
    ...
    # 4. Bespoke Grid Setup (2 rows, 3 columns)
    ...
    # Map the 5 plots to their specific subplots
    target_axes = [axes[0, 0], axes[0, 1], axes[1, 0], axes[1, 1], axes[1, 2]]
    legend_ax = axes[0, 2]

    # Hide the axes on the legend pane
    legend_ax.axis('off')
    ...
    # 7. Dynamic Y-Axis Scale (Min/Max + Epsilon)
    ...
    # 8. Legend Placement in top right corner pane
    ...
    plt.tight_layout()
    plt.savefig('gsm8k_grid.pdf', dpi=300, bbox_inches='tight')
    plt.show()
