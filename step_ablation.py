...
def main(base_cfg):
    print(f">>> Initializing Global Resources for {TARGET_TASK.upper()} Generalisation Eval...")

    params = BEST_PARAMS[TARGET_TASK]
    batch_size = 16
    ...
    for strategy_name in STRATEGIES:
        for steps in STEP_SIZES:
            for gen_length in GEN_LENGTHS:
                ...
                run_name = f"eval_{TARGET_TASK}_{strategy_name}_steps{steps}_len{gen_length}"
                run = wandb.init(
                    project=f"{TARGET_TASK}_generalisation",
